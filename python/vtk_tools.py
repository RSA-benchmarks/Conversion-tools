...
def vtkPoints(p):
    """ Creates vtk points from an numpy array
    """
...
def vtkCells(t):
    """ Creates vtk cells from an numpy array
    """
...
def get_polydata_data(polydata, data_index = 0, cell = True):
    """ The cell or vertex data from vtk.PolyData as numpy array
    """
    if cell: 
...
def get_polydata_points(polydata):
    """ The points of vtk.PolyData as numpy array
    """
...
def write_msh(name, polydata):
    """ Writes a tetraedral .msh file including cell data from vtk.PolyData
    """    
    with open(name, "w") as f:
        ...
        # Nodes        
        np_ = polydata.GetNumberOfPoints()        
        f.write("$Nodes\n")
...
def write_dgf(name, pd):    
    """ Writes a DGF file including cell data from vtk.PolyData
    """
...
if __name__ == "__main__":    
    pd = read_polydata("test.vtp")
    write_dgf("test.msh",pd)
    print("finished writing .dgf")
